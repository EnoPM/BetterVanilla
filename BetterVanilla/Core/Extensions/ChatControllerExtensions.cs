...
public static class ChatControllerExtensions
{
    ...
    {
        public void AddPrivateChat(PlayerControl sender, PlayerControl receiver, string chatText)
        {
            ...
            try
            {
                ...
                var color = colorId < Palette.PlayerColors.Count
                    ? Palette.PlayerColors[colorId]
                    : Palette.PlayerColors[0];
                ...
                if (num != 0) return;
                SoundManager.Instance.PlaySound(controller.messageSound, false).pitch =
                    (float)(0.5 + sender.PlayerId / 15.0);
            }
            catch (Exception ex)
            {
