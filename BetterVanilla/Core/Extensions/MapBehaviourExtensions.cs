...
public static class MapBehaviourExtensions
{
    ...
    extension(MapBehaviour mapBehaviour)
    {
        public static void BetterShow(MapOptions opts)
        {
            MapMode = opts.Mode;
        }
        ...
        private void UpdateOrCreateAllPlayerIcons()
        {
        ...
        private void UpdateOrCreateAllVentIcons()
        {
            var shipStatus = ShipStatus.Instance;
            if (!shipStatus) return;
            if (AllVents.Count > 0)
        ...
        private void DisableAllVentIcons()
        {
            foreach (var (_, sr) in AllVents)
            {
                if (!sr) continue;
