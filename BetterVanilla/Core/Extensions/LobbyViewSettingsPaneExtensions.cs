...
{
    extension(LobbyViewSettingsPane pane)
    {
        public void DrawBetterNormalTab()
        {
            var y1 = 1.44f;
            foreach (var allCategory in GameManager.Instance.GetAllCategories())
            {
                ...
                for (var index = 0; index < allCategory.AllGameSettings.Count; ++index)
                {
                    ...
                    var customOption = HostOptions.Default.GetOptions().FirstOrDefault(x =>
                        x is IBaseHostOption hostOption && hostOption.GetGameSetting() == gameSetting);
                    ...
                    var num = customOption == null
                        ? GameOptionsManager.Instance.CurrentGameOptions.GetValue(gameSetting)
                        : -1f;
